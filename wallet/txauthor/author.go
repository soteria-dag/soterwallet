...
import (
	...
	h "github.com/soteria-dag/soterwallet/internal/helpers"
	"github.com/soteria-dag/soterwallet/wallet/internal/txsizes"
)
...
// InputSourceError.
type InputSource func(target soterutil.Amount) (total soterutil.Amount, inputs []*wire.TxIn,
	inputValues []soterutil.Amount, scripts [][]byte, err error)
...
type AuthoredTx struct {
	Tx              *wire.MsgTx
	PrevScripts     [][]byte
	PrevInputValues []soterutil.Amount
	TotalInput      soterutil.Amount
...
func NewUnsignedTransaction(outputs []*wire.TxOut, relayFeePerKb soterutil.Amount,
...
func AddAllInputScripts(tx *wire.MsgTx, prevPkScripts [][]byte, inputValues []soterutil.Amount,
...
func spendWitnessKeyHash(txIn *wire.TxIn, pkScript []byte,
	...
	p2wkhAddr, err := soterutil.NewAddressWitnessPubKeyHash(pubKeyHash, chainParams)
	if err != nil {
		return err
	}
