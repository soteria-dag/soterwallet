...
func (w *Wallet) MakeMultiSigScript(addrs []soterutil.Address, nRequired int) ([]byte, error) {
	pubKeys := make([]*soterutil.AddressPubKey, len(addrs))
...
func (w *Wallet) ImportP2SHRedeemScript(script []byte) (*soterutil.AddressScriptHash, error) {
	var p2shAddr *soterutil.AddressScriptHash
	err := walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		...
		if err != nil {
			...
			if waddrmgr.IsError(err, waddrmgr.ErrDuplicateAddress) {
				...
				p2shAddr, _ = soterutil.NewAddressScriptHash(script,
					w.chainParams)
				return nil
			}
			return err
		...
		p2shAddr = addrInfo.Address().(*soterutil.AddressScriptHash)
		return nil
	})
	return p2shAddr, err
}
