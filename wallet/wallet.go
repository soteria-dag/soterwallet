...
import (
	...
	"github.com/soteria-dag/soterwallet/wallet/txauthor"
	"github.com/soteria-dag/soterwallet/wallet/txrules"
	"github.com/soteria-dag/soterwallet/walletdb"
	"github.com/soteria-dag/soterwallet/walletdb/migration"
...
// rescan request.
func (w *Wallet) activeData(dbtx walletdb.ReadTx) ([]soterutil.Address, []wtxmgr.Credit, error) {
	addrmgrNs := dbtx.ReadBucket(waddrmgrNamespaceKey)
	txmgrNs := dbtx.ReadBucket(wtxmgrNamespaceKey)

	var addrs []soterutil.Address
	err := w.Manager.ForEachActiveAddress(addrmgrNs, func(addr soterutil.Address) error {
...
func (w *Wallet) syncWithChain(birthdayStamp *waddrmgr.BlockStamp) error {
	...
	OUTER:
		for height := rollbackStamp.Height; true; height-- {
			...
			chainHashes, err := chainClient.GetBlockHash(int64(height))
			if err != nil {
				return err
			}
...
func (w *Wallet) scanChain(startHeight int32,
	...
	// chain backend is currently synced. When running with a soterd or
	...
	for height := startHeight; height <= bestHeight; height++ {
		hashes, err := chainClient.GetBlockHash(int64(height))
		if err != nil {
			return err
		}
...
func newFilterBlocksRequest(batch []wtxmgr.BlockMeta,
	...
	filterReq := &chain.FilterBlocksRequest{
		Blocks:           batch,
		ExternalAddrs:    make(map[waddrmgr.ScopedIndex]soterutil.Address),
		InternalAddrs:    make(map[waddrmgr.ScopedIndex]soterutil.Address),
...
func (w *Wallet) CreateSimpleTx(account uint32, outputs []*wire.TxOut,
	minconf int32, satPerKb soterutil.Amount, dryRun bool) (
...
// account.
func (w *Wallet) AccountAddresses(account uint32) (addrs []soterutil.Address, err error) {
	err = walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
...
// include a UTXO.
func (w *Wallet) CalculateBalance(confirms int32) (soterutil.Amount, error) {
...
type Balances struct {
	Total          soterutil.Amount
	Spendable      soterutil.Amount
	ImmatureReward soterutil.Amount
}
...
// CurrentAddress gets the most recently requested soter payment address
...
// blockchain or soterd mempool), the next chained address is returned.
func (w *Wallet) CurrentAddress(account uint32, scope waddrmgr.KeyScope) (soterutil.Address, error) {
	chainClient, err := w.requireChainClient()
	if err != nil {
		return nil, err
	}
	...
	if props != nil {
		err = chainClient.NotifyReceived([]soterutil.Address{addr})
		if err != nil {
			return nil, err
		}
...
func (w *Wallet) PubKeyForAddress(a soterutil.Address) (*soterec.PublicKey, error) {
	var pubKey *soterec.PublicKey
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
...
// address.
func (w *Wallet) PrivKeyForAddress(a soterutil.Address) (*soterec.PrivateKey, error) {
	var privKey *soterec.PrivateKey
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
		managedAddr, err := w.Manager.Address(addrmgrNs, a)
		if err != nil {
...
func (w *Wallet) HaveAddress(a soterutil.Address) (bool, error) {
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
...
func (w *Wallet) AccountOfAddress(a soterutil.Address) (uint32, error) {
	var account uint32
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
		var err error
...
func (w *Wallet) AddressInfo(a soterutil.Address) (waddrmgr.ManagedAddress, error) {
...
func listTransactions(tx walletdb.ReadTx, details *wtxmgr.TxDetails, addrMgr *waddrmgr.Manager,
	syncHeight int32, net *chaincfg.Params) []soterjson.ListTransactionsResult {
	...
	results := []soterjson.ListTransactionsResult{}
	...
	if len(details.Debits) == len(details.MsgTx.TxIn) {
		var debitTotal soterutil.Amount
...
func (w *Wallet) ListSinceBlock(start, end, syncHeight int32) ([]soterjson.ListTransactionsResult, error) {
	txList := []soterjson.ListTransactionsResult{}
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
...
// replies.
func (w *Wallet) ListTransactions(from, count int) ([]soterjson.ListTransactionsResult, error) {
	txList := []soterjson.ListTransactionsResult{}
...
func (w *Wallet) ListAddressTransactions(pkHashes map[string]struct{}) ([]soterjson.ListTransactionsResult, error) {
	txList := []soterjson.ListTransactionsResult{}
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
		...
		loopDetails:
			for i := range details {
				...
				for _, cred := range detail.Credits {
					...
					apkh, ok := addrs[0].(*soterutil.AddressPubKeyHash)
					if !ok {
						continue
					}
...
// replies.
func (w *Wallet) ListAllTransactions() ([]soterjson.ListTransactionsResult, error) {
	txList := []soterjson.ListTransactionsResult{}
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
...
func (w *Wallet) Accounts(scope waddrmgr.KeyScope) (*AccountsResult, error) {
	...
	err = walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		m := make(map[uint32]*soterutil.Amount)
...
type AccountBalanceResult struct {
	...
	AccountName    string
	AccountBalance soterutil.Amount
}
...
func (w *Wallet) ListUnspent(minconf, maxconf int32,
	addresses map[string]struct{}) ([]*soterjson.ListUnspentResult, error) {

	var results []*soterjson.ListUnspentResult
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
		...
		results = make([]*soterjson.ListUnspentResult, 0, len(unspent))
		for i := range unspent {
			output := unspent[i]
		...
		scSwitch:
			...
			result := &soterjson.ListUnspentResult{
...
func (w *Wallet) DumpPrivKeys() ([]string, error) {
	...
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		// privkeys.
		return w.Manager.ForEachActiveAddress(addrmgrNs, func(addr soterutil.Address) error {
...
func (w *Wallet) DumpWIFPrivateKey(addr soterutil.Address) (string, error) {
...
func (w *Wallet) ImportPrivateKey(scope waddrmgr.KeyScope, wif *soterutil.WIF,
	...
	if rescan {
		...
		err := w.chainClient.NotifyReceived([]soterutil.Address{addr})
		if err != nil {
...
func (w *Wallet) LockedOutpoints() []soterjson.TransactionInput {
	locked := make([]soterjson.TransactionInput, len(w.lockedOutpoints))
	i := 0
	for op := range w.lockedOutpoints {
		locked[i] = soterjson.TransactionInput{
			Txid: op.Hash.String(),
			Vout: op.Index,
		}
		i++
...
func (w *Wallet) NewAddress(account uint32,
	scope waddrmgr.KeyScope) (soterutil.Address, error) {
...
func (w *Wallet) newChangeAddress(addrmgrNs walletdb.ReadWriteBucket,
	account uint32) (soterutil.Address, error) {
...
// returning the total amount of soter received for a single wallet
// address.
func (w *Wallet) TotalReceivedForAddr(addr soterutil.Address, minConf int32) (soterutil.Amount, error) {
	var amount soterutil.Amount
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
...
func (w *Wallet) SendOutputs(outputs []*wire.TxOut, account uint32,
	minconf int32, satPerKb soterutil.Amount) (*wire.MsgTx, error) {
...
func (w *Wallet) SignTransaction(tx *wire.MsgTx, hashType txscript.SigHashType,
	...
	additionalKeysByAddress map[string]*soterutil.WIF,
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		for i, txIn := range tx.TxIn {
			...
			getKey := txscript.KeyClosure(func(addr soterutil.Address) (*soterec.PrivateKey, bool, error) {
			...
			getScript := txscript.ScriptClosure(func(addr soterutil.Address) ([]byte, error) {
...
func (w *Wallet) publishTransaction(tx *wire.MsgTx) (*chainhash.Hash, error) {
	...
	// soterd node that already has it in their mempool.
	...
	// has already confirmed to a soterd node.
...
// in order to allow applications wrapping soterwallet to store app-specific data
