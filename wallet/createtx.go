...
func makeInputSource(eligible []wtxmgr.Credit) txauthor.InputSource {
	...
	currentInputValues := make([]soterutil.Amount, 0, len(eligible))

	return func(target soterutil.Amount) (soterutil.Amount, []*wire.TxIn,
		[]soterutil.Amount, [][]byte, error) {
...
func (s secretSource) GetKey(addr soterutil.Address) (*soterec.PrivateKey, bool, error) {
...
func (s secretSource) GetScript(addr soterutil.Address) ([]byte, error) {
	ma, err := s.Address(s.addrmgrNs, addr)
	if err != nil {
		return nil, err
	}
...
func (w *Wallet) txToOutputs(outputs []*wire.TxOut, account uint32,
	minconf int32, feeSatPerKb soterutil.Amount, dryRun bool) (
	...
	changeSource := func() ([]byte, error) {
		...
		var changeAddr soterutil.Address
		var err error
...
func validateMsgTx(tx *wire.MsgTx, prevScripts [][]byte, inputValues []soterutil.Amount) error {
