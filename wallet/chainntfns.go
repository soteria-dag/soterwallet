...
func (w *Wallet) handleChainNotifications() {
	...
	catchUpHashes := func(w *Wallet, client chain.Interface,
		...
		// call. The solution when using soterd is to make soterd
		...
		err := walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
			...
			for i := startBlock.Height + 1; i <= height; i++ {
				hashes, err := client.GetBlockHash(int64(i))
				if err != nil {
					return err
				}
...
type chainConn interface {
	...
	// GetBlockHash returns the hashes of the blocks with the given height.
...
func birthdaySanityCheck(chainConn chainConn,
	...
	for birthdayTimestamp.Before(candidate.Timestamp) {
		...
		hashes, err := chainConn.GetBlockHash(newCandidateHeight)
		if err != nil {
			return nil, fmt.Errorf("unable to get block hashes for "+
...
OUTER:
	for timestampDelta > birthdayBlockDelta {
		...
		// timestamp.
		hashes, err := chainConn.GetBlockHash(int64(newHeight))
		if err != nil {
			return nil, fmt.Errorf("unable to get block hashes for "+
				"height %d: %v", candidate.Height, err)
		}
