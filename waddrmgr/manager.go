...
// would end up being the same soter address (as is the case for
...
func (m *Manager) MarkUsed(ns walletdb.ReadWriteBucket, address soterutil.Address) error {
	m.mtx.RLock()
	defer m.mtx.RUnlock()
...
func (m *Manager) AddrAccount(ns walletdb.ReadBucket,
	address soterutil.Address) (*ScopedKeyManager, uint32, error) {
...
func (m *Manager) ForEachActiveAddress(ns walletdb.ReadBucket, fn func(addr soterutil.Address) error) error {
	m.mtx.RLock()
	defer m.mtx.RUnlock()
