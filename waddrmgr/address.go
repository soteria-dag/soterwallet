...
import (
	...
	"github.com/soteria-dag/soterwallet/internal/zero"
...
// style of soter addresses.  It additionally provides information about the
// script.
...
// Address returns the soterutil.Address which represents the managed address.
// This will be a pay-to-pubkey-hash address.
//
// This is part of the ManagedAddress interface implementation.
func (a *managedAddress) Address() soterutil.Address {
	return a.address
}
...
func (a *managedAddress) AddrHash() []byte {
	...
	case *soterutil.AddressPubKeyHash:
		hash = n.Hash160()[:]
	case *soterutil.AddressScriptHash:
		hash = n.Hash160()[:]
	case *soterutil.AddressWitnessPubKeyHash:
		hash = n.Hash160()[:]
	}
...
func (a *managedAddress) PubKey() *soterec.PublicKey {
	return a.pubKey
}
...
//
// This is part of the ManagedPubKeyAddress interface implementation.
func (a *managedAddress) PrivKey() (*soterec.PrivateKey, error) {
	...
	privKey, _ := soterec.PrivKeyFromBytes(soterec.S256(), privKeyCopy)
...
//
// This is part of the ManagedPubKeyAddress interface implementation.
func (a *managedAddress) ExportPrivKey() (*soterutil.WIF, error) {
	pk, err := a.PrivKey()
	if err != nil {
		return nil, err
	}

	return soterutil.NewWIF(pk, a.manager.rootManager.chainParams, a.compressed)
}
...
func newManagedAddressWithoutPrivKey(m *ScopedKeyManager,
	derivationPath DerivationPath, pubKey *soterec.PublicKey, compressed bool,
	addrType AddressType) (*managedAddress, error) {

	// Create a pay-to-pubkey-hash address from the public key.
	var pubKeyHash []byte
	if compressed {
		pubKeyHash = soterutil.Hash160(pubKey.SerializeCompressed())
	} else {
		pubKeyHash = soterutil.Hash160(pubKey.SerializeUncompressed())
	}
	...
	case NestedWitnessPubKey:
		...
		// backwards compatible to soter nodes running 0.6.0 onwards, but
		...
		witAddr, err := soterutil.NewAddressWitnessPubKeyHash(
		...
		address, err = soterutil.NewAddressScriptHash(
	...
	case PubKeyHash:
		address, err = soterutil.NewAddressPubKeyHash(
			pubKeyHash, m.rootManager.chainParams,
		)
		if err != nil {
			return nil, err
	...
	case WitnessPubKey:
		address, err = soterutil.NewAddressWitnessPubKeyHash(
			pubKeyHash, m.rootManager.chainParams,
		)
		if err != nil {
			return nil, err
...
func newManagedAddress(s *ScopedKeyManager, derivationPath DerivationPath,
	privKey *soterec.PrivateKey, compressed bool,
	addrType AddressType) (*managedAddress, error) {
	...
	ecPubKey := (*soterec.PublicKey)(&privKey.PublicKey)
...
type scriptAddress struct {
	manager         *ScopedKeyManager
	account         uint32
	address         *soterutil.AddressScriptHash
...
//
// This is part of the ManagedAddress interface implementation.
func (a *scriptAddress) Address() soterutil.Address {
	return a.address
}
...
func newScriptAddress(m *ScopedKeyManager, account uint32, scriptHash,
	...
	address, err := soterutil.NewAddressScriptHashFromHash(
