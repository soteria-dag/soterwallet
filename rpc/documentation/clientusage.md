...
(account 0) from a testnet1 wallet listening on `localhost:18332` in several
different languages:
...
auto main() -> int {
    ...
    if (!status.ok()) {
        ...
        std::cout << "Spendable balance: " << response.spendable() << " nanoSoter" << std::endl;
    }
}
```
    ...
    static class Program
    {
        ...
        static async Task ExampleAsync()
        {
            ...
            try
            {
                ...
                Console.WriteLine($"Spendable balance: {balanceResponse.Spendable} nanoSoter");
            }
            finally
            {
...
client.balance(request, function(err, response) {
    if (err) {
        ...
        console.log('Spendable balance:', response.spendable, 'nanoSoter');
    }
});
```
...
def main():
    ...
    print 'Spendable balance: %d nanoSoter' % response.spendable
