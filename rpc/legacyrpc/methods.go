...
import (
	...
	"github.com/soteria-dag/soterwallet/wallet/txrules"
...
// request into a marshalable response.  If the error is a *soterjson.RPCError
...
// catch-all error code, soterjson.ErrRPCWallet.
...
var rpcHandlers = map[string]struct {
	...
	// Reference methods which can't be implemented by soterwallet due to
...
func unimplemented(interface{}, *wallet.Wallet) (interface{}, error) {
	return nil, &soterjson.RPCError{
		Code:    soterjson.ErrRPCUnimplemented,
		Message: "Method unimplemented",
	}
}

// unsupported handles a standard bitcoind RPC request which is
// unsupported by soterwallet due to design differences.
func unsupported(interface{}, *wallet.Wallet) (interface{}, error) {
	return nil, &soterjson.RPCError{
		Code:    -1,
		Message: "Request unsupported by soterwallet",
	}
}
...
// context.
type lazyHandler func() (interface{}, *soterjson.RPCError)
...
func lazyApplyHandler(request *soterjson.Request, w *wallet.Wallet, chainClient chain.Interface) lazyHandler {
	handlerData, ok := rpcHandlers[request.Method]
	if ok && handlerData.handlerWithChain != nil && w != nil && chainClient != nil {
		return func() (interface{}, *soterjson.RPCError) {
			cmd, err := soterjson.UnmarshalCmd(request)
			if err != nil {
				return nil, soterjson.ErrRPCInvalidRequest
			}
...
func makeResponse(id, result interface{}, err error) soterjson.Response {
	idPtr := idPointer(id)
	if err != nil {
		return soterjson.Response{
			ID:    idPtr,
			Error: jsonError(err),
		}
	}
...
func jsonError(err error) *soterjson.RPCError {
	if err == nil {
		return nil
	}
	...
	case ParseError:
		code = soterjson.ErrRPCParse.Code
	case waddrmgr.ManagerError:
		switch e.ErrorCode {
		case waddrmgr.ErrWrongPassphrase:
			code = soterjson.ErrRPCWalletPassphraseIncorrect
		}
	}
	return &soterjson.RPCError{
		Code:    code,
		Message: err.Error(),
	}
}
...
func makeMultiSigScript(w *wallet.Wallet, keys []string, nRequired int) ([]byte, error) {
	keysesPrecious := make([]*soterutil.AddressPubKey, len(keys))
	...
	for i, a := range keys {
		...
		default:
			...
			pubKeyAddr, err := soterutil.NewAddressPubKey(
...
func addMultiSigAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*soterjson.AddMultisigAddressCmd)
	...
	secp256k1Addrs := make([]soterutil.Address, len(cmd.Keys))
...
func createMultiSig(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*soterjson.CreateMultisigCmd)
	...
	address, err := soterutil.NewAddressScriptHash(script, w.ChainParams())
	if err != nil {
		// above is a valid script, shouldn't happen.
		return nil, err
	}

	return soterjson.CreateMultiSigResult{
...
func dumpPrivKey(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*soterjson.DumpPrivKeyCmd)
...
func getAddressesByAccount(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*soterjson.GetAddressesByAccountCmd)
...
func getBalance(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*soterjson.GetBalanceCmd)
...
func getBestBlock(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	result := &soterjson.GetBestBlockResult{
...
// information about the current state of soterwallet.
// exist.
func getInfo(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	// Call down to soterd for all of the information in this command known
	// by them.
...
func decodeAddress(s string, params *chaincfg.Params) (soterutil.Address, error) {
	addr, err := soterutil.DecodeAddress(s, params)
	if err != nil {
...
func getAccount(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*soterjson.GetAccountCmd)
...
// appear in the blockchain, or any tx that has arrived in the soterd mempool).
// If the most recently-requested address has been used, a new address (the
// next chained address in the keypool) is used.  This can fail if the keypool
// runs out (and will return soterjson.ErrRPCWalletKeypoolRanOut if that happens).
func getAccountAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*soterjson.GetAccountAddressCmd)
...
func getUnconfirmedBalance(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*soterjson.GetUnconfirmedBalanceCmd)
...
func importPrivKey(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	wif, err := soterutil.DecodeWIF(cmd.PrivKey)
	if err != nil {
		return nil, &soterjson.RPCError{
			Code:    soterjson.ErrRPCInvalidAddressOrKey,
...
func createNewAccount(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*soterjson.CreateNewAccountCmd)
...
func renameAccount(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*soterjson.RenameAccountCmd)
...
func getNewAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*soterjson.GetNewAddressCmd)
...
func getRawChangeAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*soterjson.GetRawChangeAddressCmd)
...
func getReceivedByAccount(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*soterjson.GetReceivedByAccountCmd)
	...
	return results[acctIndex].TotalReceived.ToSOTER(), nil
}
...
func getReceivedByAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*soterjson.GetReceivedByAddressCmd)
...
func getTransaction(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	ret := soterjson.GetTransactionResult{
		TxID:            cmd.Txid,
	...
	var (
		debitTotal  soterutil.Amount
		creditTotal soterutil.Amount // Excludes change
		fee         soterutil.Amount
		feeF64      float64
	)
	...
	if len(details.Debits) == len(details.MsgTx.TxIn) {
		var outputTotal soterutil.Amount
		for _, output := range details.MsgTx.TxOut {
			outputTotal += soterutil.Amount(output.Value)
		}
	...
	if len(details.Debits) == 0 {
		...
		ret.Details = make([]soterjson.GetTransactionDetailsResult, 0, len(details.Credits))
	} else {
		ret.Details = make([]soterjson.GetTransactionDetailsResult, 1, len(details.Credits)+1)

		ret.Details[0] = soterjson.GetTransactionDetailsResult{
			// Fields left zeroed:
			//   InvolvesWatchOnly
			//   Account
			//   Address
			...
			Category: "send",
			Amount:   (-debitTotal).ToSOTER(), // negative since it is a send
			Fee:      &feeF64,
		}
		ret.Fee = feeF64
	}
	...
	for _, cred := range details.Credits {
		...
		ret.Details = append(ret.Details, soterjson.GetTransactionDetailsResult{
			// Fields left zeroed:
			//   InvolvesWatchOnly
			//   Fee
			Account:  accountName,
			Address:  address,
			Category: credCat,
			Amount:   cred.Amount.ToSOTER(),
			Vout:     cred.Index,
		})
	}

	ret.Amount = creditTotal.ToSOTER()
	return ret, nil
}
...
func help(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	cmd := icmd.(*soterjson.HelpCmd)

	// soterd returns different help messages depending on the kind of
	...
	// wallet itself is a websocket client to soterd.  Therefore, create a
	...
	// texts in a non-internal soterd package.
...
func listAccounts(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*soterjson.ListAccountsCmd)
	...
	for _, result := range results {
		accountBalances[result.AccountName] = result.AccountBalance.ToSOTER()
	}
...
func listReceivedByAccount(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*soterjson.ListReceivedByAccountCmd)
	...
	jsonResults := make([]soterjson.ListReceivedByAccountResult, 0, len(results))
	for _, result := range results {
		jsonResults = append(jsonResults, soterjson.ListReceivedByAccountResult{
			Account:       result.AccountName,
			Amount:        result.TotalReceived.ToSOTER(),
...
func listReceivedByAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*soterjson.ListReceivedByAddressCmd)
	...
	ret := make([]soterjson.ListReceivedByAddressResult, numAddresses, numAddresses)
	idx := 0
	for address, addrData := range allAddrData {
		ret[idx] = soterjson.ListReceivedByAddressResult{
			Address:       address,
			Amount:        addrData.amount.ToSOTER(),
...
func listSinceBlock(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	cmd := icmd.(*soterjson.ListSinceBlockCmd)
	...
	for _, hash := range hashes {
		lastBlockHash = hash.String()
	}

	res := soterjson.ListSinceBlockResult{
...
func listTransactions(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*soterjson.ListTransactionsCmd)
...
func listAddressTransactions(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*soterjson.ListAddressTransactionsCmd)
...
func listAllTransactions(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*soterjson.ListAllTransactionsCmd)
...
func listUnspent(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*soterjson.ListUnspentCmd)
...
func lockUnspent(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*soterjson.LockUnspentCmd)
...
// and amounts.
func makeOutputs(pairs map[string]soterutil.Amount, chainParams *chaincfg.Params) ([]*wire.TxOut, error) {
	outputs := make([]*wire.TxOut, 0, len(pairs))
	for addrStr, amt := range pairs {
		addr, err := soterutil.DecodeAddress(addrStr, chainParams)
		if err != nil {
...
// All errors are returned in soterjson.RPCError format
func sendPairs(w *wallet.Wallet, amounts map[string]soterutil.Amount,
	account uint32, minconf int32, feeSatPerKb soterutil.Amount) (string, error) {
...
func sendFrom(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	cmd := icmd.(*soterjson.SendFromCmd)
	...
	amt, err := soterutil.NewAmount(cmd.Amount)
	if err != nil {
		return nil, err
	}
	pairs := map[string]soterutil.Amount{
		cmd.ToAddress: amt,
	}
...
func sendMany(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*soterjson.SendManyCmd)
	...
	pairs := make(map[string]soterutil.Amount, len(cmd.Amounts))
	for k, v := range cmd.Amounts {
		amt, err := soterutil.NewAmount(v)
		if err != nil {
			return nil, err
		}
		pairs[k] = amt
...
func sendToAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*soterjson.SendToAddressCmd)
...
func setTxFee(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*soterjson.SetTxFeeCmd)
...
func signMessage(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*soterjson.SignMessageCmd)
	...
	wire.WriteVarString(&buf, 0, "Soter Signed Message:\n")
	wire.WriteVarString(&buf, 0, cmd.Message)
	messageHash := chainhash.DoubleHashB(buf.Bytes())
	sigbytes, err := soterec.SignCompact(soterec.S256(), privKey,
		messageHash, true)
	if err != nil {
		return nil, err
	}
...
func signRawTransaction(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	cmd := icmd.(*soterjson.SignRawTransactionCmd)
	...
	// TODO: really we probably should look these up with soterd anyway to
	// make sure that they match the blockchain if present.
	inputs := make(map[wire.OutPoint][]byte)
	scripts := make(map[string][]byte)
	var cmdInputs []soterjson.RawTxInput
	if cmd.Inputs != nil {
	...
	for _, rti := range cmdInputs {
		...
		if cmd.PrivKeys != nil && len(*cmd.PrivKeys) != 0 {
			...
			addr, err := soterutil.NewAddressScriptHash(redeemScript,
				w.ChainParams())
			if err != nil {
				return nil, DeserializationError{err}
			}
	...
	// querying soterd with getrawtransaction. We queue up a bunch of async
	...
	var keys map[string]*soterutil.WIF
	if cmd.PrivKeys != nil {
		keys = make(map[string]*soterutil.WIF)

		for _, key := range *cmd.PrivKeys {
			wif, err := soterutil.DecodeWIF(key)
			if err != nil {
				return nil, DeserializationError{err}
			}
			...
			addr, err := soterutil.NewAddressPubKey(wif.SerializePubKey(),
				w.ChainParams())
			if err != nil {
				return nil, DeserializationError{err}
			}
	...
	signErrors := make([]soterjson.SignRawTransactionError, 0, len(signErrs))
	for _, e := range signErrs {
		input := tx.TxIn[e.InputIndex]
		signErrors = append(signErrors, soterjson.SignRawTransactionError{
	...
	return soterjson.SignRawTransactionResult{
...
func validateAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*soterjson.ValidateAddressCmd)

	result := soterjson.ValidateAddressWalletResult{}
	addr, err := decodeAddress(cmd.Address, w.ChainParams())
	if err != nil {
...
func verifyMessage(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*soterjson.VerifyMessageCmd)
	...
	pk, wasCompressed, err := soterec.RecoverCompact(soterec.S256(), sig,
		expectedMessageHash)
	if err != nil {
		return nil, err
	}
	...
	case *soterutil.AddressPubKeyHash: // ok
		return bytes.Equal(soterutil.Hash160(serializedPubKey), checkAddr.Hash160()[:]), nil
	case *soterutil.AddressPubKey: // ok
...
func walletPassphrase(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*soterjson.WalletPassphraseCmd)
...
func walletPassphraseChange(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*soterjson.WalletPassphraseChangeCmd)
