...
func jsonAuthFail(w http.ResponseWriter) {
	w.Header().Add("WWW-Authenticate", `Basic realm="soterwallet RPC"`)
...
// functional soter wallet RPC server.  This can be called to enable RPC
...
// method.  This may be a request that is handled directly by soterwallet, or
// a chain server request that is handled by passing the request down to soterd.
//
...
func (s *Server) handlerClosure(request *soterjson.Request) lazyHandler {
	s.handlerMu.Lock()
...
func sanitizeRequest(r *soterjson.Request) string {
...
// but this is only implemented here to work around an oddity with soterjson,
...
func (s *Server) invalidAuth(req *soterjson.Request) bool {
	cmd, err := soterjson.UnmarshalCmd(req)
	if err != nil {
		return false
	}
	authCmd, ok := cmd.(*soterjson.AuthenticateCmd)
	if !ok {
		return false
	}
	// Check credentials.
		...
		case reqBytes, ok := <-wsc.allRequests:
			...
			if err != nil {
				...
				resp := makeResponse(req.ID, nil,
					soterjson.ErrRPCInvalidRequest)
			...
			default:
				...
				go func() {
					resp, jsonErr := f()
					mresp, err := soterjson.MarshalResponse(req.ID, resp, jsonErr)
					if err != nil {
...
func (s *Server) postClientRPC(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		resp, err := soterjson.MarshalResponse(req.ID, nil, soterjson.ErrRPCInvalidRequest)
		if err != nil {
			log.Errorf("Unable to marshal response: %v", err)
	...
	var res interface{}
	var jsonErr *soterjson.RPCError
	var stop bool
	switch req.Method {
	case "authenticate":
		// Drop it.
	...
	case "stop":
		stop = true
		res = "soterwallet stopping"
	default:
		res, jsonErr = s.handlerClosure(&req)()
	}

	// Marshal and send.
	mresp, err := soterjson.MarshalResponse(req.ID, res, jsonErr)
	if err != nil {
		log.Errorf("Unable to marshal response: %v", err)
