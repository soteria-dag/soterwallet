...
//
//   https://github.com/soteria-dag/soterwallet/blob/master/rpc/documentation/api.md
//
// Any API changes must be performed according to the steps listed here:
//
//   https://github.com/soteria-dag/soterwallet/blob/master/rpc/documentation/serverchanges.md
package rpcserver
...
import (
	...
	pb "github.com/soteria-dag/soterwallet/rpc/walletrpc"
...
// as well as establishing a RPC connection to a soterd consensus server.
...
func (s *walletServer) ImportPrivateKey(ctx context.Context, req *pb.ImportPrivateKeyRequest) (
	*pb.ImportPrivateKeyResponse, error) {
	...
	wif, err := soterutil.DecodeWIF(req.PrivateKeyWif)
	if err != nil {
...
func (s *walletServer) FundTransaction(ctx context.Context, req *pb.FundTransactionRequest) (
	*pb.FundTransactionResponse, error) {
	...
	var totalAmount soterutil.Amount
	for _, output := range unspentOutputs {
		...
		totalAmount += soterutil.Amount(output.Output.Value)

		if req.TargetAmount != 0 && totalAmount > soterutil.Amount(req.TargetAmount) {
			break
		}
	}

	var changeScript []byte
	if req.IncludeChangeScript && totalAmount > soterutil.Amount(req.TargetAmount) {
...
//   sendrawtransaction, so connection errors to soterd could result in the tx
