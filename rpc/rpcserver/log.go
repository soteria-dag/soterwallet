...
func UseLogger(l soterlog.Logger) {
	grpclog.SetLogger(logger{l})
}

// logger uses a soterlog.Logger to implement the grpclog.Logger interface.
type logger struct {
	soterlog.Logger
}

// stripGrpcPrefix removes the package prefix for all logs made to the grpc
// logger, since these are already included as the soterlog subsystem name.
