...
import (
	...
	"github.com/soteria-dag/soterwallet/internal/cfgutil"
	"github.com/soteria-dag/soterwallet/internal/legacy/keystore"
	"github.com/soteria-dag/soterwallet/netparams"
...
const (
	defaultCAFilename       = "soterd.cert"
	defaultConfigFilename   = "soterwallet.conf"
	defaultLogLevel         = "info"
	defaultLogDirname       = "logs"
	defaultLogFilename      = "soterwallet.log"
...
var (
	soterdDefaultCAFile = filepath.Join(soterutil.AppDataDir("soterd", false), "rpc.cert")
	defaultAppDataDir   = soterutil.AppDataDir("soterwallet", false)
...
type config struct {
	...
	TestNet1      bool                    `long:"testnet" description:"Use the test soter network (version 1) (default mainnet)"`
	...
	// RPC client options
	RPCConnect       string                  `short:"c" long:"rpcconnect" description:"Hostname/IP and port of soterd RPC server to connect to (default localhost:8334, testnet: localhost:18334, simnet: localhost:18556)"`
	CAFile           *cfgutil.ExplicitString `long:"cafile" description:"File containing root certificates to authenticate a TLS connections with soterd"`
	DisableClientTLS bool                    `long:"noclienttls" description:"Disable TLS for the RPC client -- NOTE: This is only allowed if the RPC client is connecting to localhost"`
	SoterdUsername   string                  `long:"soterdusername" description:"Username for soterd authentication"`
	SoterdPassword   string                  `long:"soterdpassword" default-mask:"-" description:"Password for soterd authentication"`
	...
	Username               string                  `short:"u" long:"username" description:"Username for legacy RPC and soterd authentication (if soterdusername is unset)"`
	Password               string                  `short:"P" long:"password" default-mask:"-" description:"Password for legacy RPC and soterd authentication (if soterdpassword is unset)"`
...
//
// The above results in soterwallet functioning properly without any config
...
func loadConfig() (*config, []string, error) {
	...
	numNets := 0
	if cfg.TestNet1 {
		activeNet = &netparams.TestNet1Params
		numNets++
	}
	if cfg.SimNet {
	...
	// simnet or testnet1.
	if !cfg.SimNet && !cfg.TestNet1 && cfg.CreateTemp {
		fmt.Fprintln(os.Stderr, "Tried to create a temporary "+
			"wallet for network other than simnet|testnet!")
		os.Exit(0)
	}
	...
	if cfg.DisableClientTLS {
		...
		// If CAFile is unset, choose either the copy or local soterd cert.
		if !cfg.CAFile.ExplicitlySet() {
			cfg.CAFile.Value = filepath.Join(cfg.AppDataDir.Value, defaultCAFilename)

			// If the CA copy does not exist, check if we're connecting to
			// a local soterd and switch to its RPC cert if it exists.
			...
			if !certExists {
				if _, ok := localhostListeners[RPCHost]; ok {
					soterdCertExists, err := cfgutil.FileExists(
						soterdDefaultCAFile)
					if err != nil {
						fmt.Fprintln(os.Stderr, err)
						return nil, nil, err
					...
					if soterdCertExists {
						cfg.CAFile.Value = soterdDefaultCAFile
					}
				}
			}
		}
	...
	// If the soterd username or password are unset, use the same auth as for
	// the client.  The two settings were previously shared for soterd and
	// client auth, so this avoids breaking backwards compatibility while
	// allowing users to use different auth settings for soterd and wallet.
	if cfg.SoterdUsername == "" {
		cfg.SoterdUsername = cfg.Username
	}
	if cfg.SoterdPassword == "" {
		cfg.SoterdPassword = cfg.Password
	}
