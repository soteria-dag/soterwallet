...
// the Start method. If the remote node does not operate on the same soter
...
func (c *BitcoindConn) getCurrentNet() (wire.SoterNet, error) {
	hashes, err := c.client.GetBlockHash(0)
	if err != nil {
		return 0, err
	}
	...
	for _, hash := range hashes {
		switch *hash {
		case *chaincfg.TestNet1Params.GenesisHash:
			return chaincfg.TestNet1Params.Net, nil
	...
	return 0, fmt.Errorf("no genesis hash found")
}
