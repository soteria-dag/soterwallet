...
func TestBlockFiltererOneInOneOut(t *testing.T) {
	...
	watchedOutPoints[firstTx.TxIn[0].PreviousOutPoint] = &soterutil.AddressWitnessPubKeyHash{}
	watchedOutPoints[lastTx.TxIn[0].PreviousOutPoint] = &soterutil.AddressWitnessPubKeyHash{}
...
func assertRelevantTxnsContains(t *testing.T, bf *chain.BlockFilterer, wantTx *wire.MsgTx) {
	...
	t.Fatalf("unable to find tx: %s in %d relevant txns", wantTx.TxHash(),
		len(bf.RelevantTxns))
}
