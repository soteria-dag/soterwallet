...
// RPCClient represents a persistent client connection to a soter RPC server
...
// operate on the same soter network as described by the passed chain
...
func (c *RPCClient) Rescan(startHash *chainhash.Hash, addrs []soterutil.Address,
	outPoints map[wire.OutPoint]soterutil.Address) error {
...
// soter RPC server.  This channel must be continually read or the process
...
// parseBlock parses a soterws definition of the block a tx is mined it to the
...
func parseBlock(block *soterjson.BlockDetails) (*wtxmgr.BlockMeta, error) {
	if block == nil {
		return nil, nil
	}
...
func (c *RPCClient) onRecvTx(tx *soterutil.Tx, block *soterjson.BlockDetails) {
...
func (c *RPCClient) onRedeemingTx(tx *soterutil.Tx, block *soterjson.BlockDetails) {
