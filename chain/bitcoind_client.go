...
func (c *BitcoindClient) GetBestBlock() (*chainhash.Hash, int32, error) {
	hash, height, err := c.chainConn.client.GetBestBlock()
	if err != nil {
		return nil, 0, err
	}
...
func (c *BitcoindClient) GetBlockVerbose(
	hash *chainhash.Hash) (*soterjson.GetBlockVerboseResult, error) {
...
func (c *BitcoindClient) GetBlockHeaderVerbose(
	hash *chainhash.Hash) (*soterjson.GetBlockHeaderVerboseResult, error) {
...
func (c *BitcoindClient) GetRawTransactionVerbose(
	hash *chainhash.Hash) (*soterjson.TxRawResult, error) {
...
func (c *BitcoindClient) GetTxOut(txHash *chainhash.Hash, index uint32,
	mempool bool) (*soterjson.GetTxOutResult, error) {
...
//
// NOTE: This is part of the chain.Interface interface.
func (c *BitcoindClient) NotifyReceived(addrs []soterutil.Address) error {
	c.NotifyBlocks()
...
//	[]wire.OutPoint
//	[]*wire.OutPoint
//	map[wire.OutPoint]soterutil.Address
//	[]chainhash.Hash
//	[]*chainhash.Hash
func (c *BitcoindClient) LoadTxFilter(reset bool, filters ...interface{}) error {
	...
	for _, filter := range filters {
		...
		case []soterutil.Address, []wire.OutPoint, []*wire.OutPoint,
			map[wire.OutPoint]soterutil.Address, []chainhash.Hash,
			[]*chainhash.Hash:
...
// matched as []soterjson.BlockDetails.
func (c *BitcoindClient) RescanBlocks(
	blockHashes []chainhash.Hash) ([]soterjson.RescannedBlock, error) {

	rescannedBlocks := make([]soterjson.RescannedBlock, 0, len(blockHashes))
	for _, hash := range blockHashes {
		...
		if len(relevantTxs) > 0 {
			rescannedBlock := soterjson.RescannedBlock{
				Hash: hash.String(),
			}
...
func (c *BitcoindClient) Rescan(blockHash *chainhash.Hash,
	addresses []soterutil.Address, outPoints map[wire.OutPoint]soterutil.Address) error {
...
func (c *BitcoindClient) rescanHandler() {
	...
	for {
		...
		case update := <-c.rescanUpdate:
			...
			case map[wire.OutPoint]soterutil.Address:
				c.watchMtx.Lock()
				for op := range update {
					c.watchedOutPoints[op] = struct{}{}
				}
...
func (c *BitcoindClient) onRelevantTx(tx *wtxmgr.TxRecord,
	blockDetails *soterjson.BlockDetails) {
...
// specified hash to the best known hash. This is called only within a queue processing loop.
//
// NOTE(cedric): rescan doesn't check for a 'reorg' with dag, because multiple blocks with different hashes
// can exist at the same height.
func (c *BitcoindClient) rescan(start chainhash.Hash) error {
	...
	lastHeader, err := c.GetBlockHeaderVerbose(&start)
	if err != nil {
		return err
	}
	...
	defer c.onRescanFinished(
		lastHash, lastHeader.Height,
		time.Unix(lastHeader.Time, 0),
	)

	// Cycle through blocks from the start height to the best block height
	for i := lastHeader.Height + 1; i <= bestBlock.Height; i++ {
		hashes, err := c.GetBlockHash(int64(i))
		if err != nil {
			return err
		}
		...
		for _, hash := range hashes {
			...
			afterBirthday := lastHeader.Time >= c.birthday.Unix()
			if !afterBirthday {
			...
			// Update the last block to have been examined
			blockHash := block.BlockHash()
			lastHash = &blockHash
			lastHeader = &soterjson.GetBlockHeaderVerboseResult{
			...
			if i%10000 == 0 && !notified {
				c.onRescanProgress(
					lastHash, i, block.Header.Timestamp,
				)
				notified = true
			}
		}
