...
// soterd RPC chain server, or an SPV library, as long as we write a driver for
// it.
type Interface interface {
	...
	Rescan(*chainhash.Hash, []soterutil.Address, map[wire.OutPoint]soterutil.Address) error
	NotifyReceived([]soterutil.Address) error
	NotifyBlocks() error
...
type (
	...
	FilterBlocksRequest struct {
		...
		ExternalAddrs    map[waddrmgr.ScopedIndex]soterutil.Address
		InternalAddrs    map[waddrmgr.ScopedIndex]soterutil.Address
	...
	FilterBlocksResponse struct {
		...
		FoundOutPoints     map[wire.OutPoint]soterutil.Address
