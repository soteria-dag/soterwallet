...
var (
	walletDataDirectory = soterutil.AppDataDir("soterwallet", false)
	newlineBytes        = []byte{'\n'}
)
...
// Flags.
var opts = struct {
	TestNet1              bool                `long:"testnet" description:"Use the test soter network (version 1)"`
	SimNet                bool                `long:"simnet" description:"Use the simulation soter network"`
...
func init() {
	...
	if opts.TestNet1 && opts.SimNet {
		fatalf("Multiple soter networks may not be used simultaneously")
	}
...
func makeInputSource(outputs []soterjson.ListUnspentResult) txauthor.InputSource {
	var (
		totalInputValue soterutil.Amount
		inputs          = make([]*wire.TxIn, 0, len(outputs))
		inputValues     = make([]soterutil.Amount, 0, len(outputs))
		sourceErr       error
	)
	for _, output := range outputs {
		outputAmount, err := soterutil.NewAmount(output.Amount)
		if err != nil {
	...
	return func(soterutil.Amount) (soterutil.Amount, []*wire.TxIn, []soterutil.Amount, [][]byte, error) {
...
func sweep() error {
	...
	sourceOutputs := make(map[string][]soterjson.ListUnspentResult)
	...
	var totalSwept soterutil.Amount
	var numErrors int
	...
	for _, previousOutputs := range sourceOutputs {
		...
		outputAmount := soterutil.Amount(tx.Tx.TxOut[0].Value)
...
func saneOutputValue(amount soterutil.Amount) bool {
	return amount >= 0 && amount <= soterutil.MaxNanoSoter
}

func parseOutPoint(input *soterjson.ListUnspentResult) (wire.OutPoint, error) {
