...
import (
	...
	"github.com/soteria-dag/soterwallet/internal/legacy/rename"
)
...
func pubkeyFromPrivkey(privkey []byte, compress bool) (pubkey []byte) {
	_, pk := soterec.PrivKeyFromBytes(soterec.S256(), privkey)
...
func chainedPrivKey(privkey, pubkey, chaincode []byte) ([]byte, error) {
	...
	case soterec.PubKeyBytesLenUncompressed, soterec.PubKeyBytesLenCompressed:
		// Correct length
	default:
	...
	b := t.Mod(t, soterec.S256().N).Bytes()
	return pad(32, b), nil
}
...
func chainedPubKey(pubkey, chaincode []byte) ([]byte, error) {
	var compressed bool
	switch n := len(pubkey); n {
	case soterec.PubKeyBytesLenUncompressed:
		compressed = false
	case soterec.PubKeyBytesLenCompressed:
		compressed = true
	default:
	...
	oldPk, err := soterec.ParsePubKey(pubkey, soterec.S256())
	if err != nil {
		return nil, err
	}
	newX, newY := soterec.S256().ScalarMult(oldPk.X, oldPk.Y, xorbytes)
	if err != nil {
		return nil, err
	}
	newPk := &soterec.PublicKey{
		Curve: soterec.S256(),
		X:     newX,
		Y:     newY,
	}
...
func (net *netParams) ReadFrom(r io.Reader) (int64, error) {
	...
	switch wire.SoterNet(binary.LittleEndian.Uint32(uint32Bytes)) {
	case wire.MainNet:
		*net = (netParams)(chaincfg.MainNetParams)
	case wire.TestNet1:
		*net = (netParams)(chaincfg.TestNet1Params)
	case wire.SimNet:
...
func getAddressKey(addr soterutil.Address) addressKey {
...
type Store struct {
	...
	passphrase       []byte
	secret           []byte
	chainIdxMap      map[int64]soterutil.Address
...
func New(dir string, desc string, passphrase []byte, net *chaincfg.Params,
	...
	s := &Store{
		...
		chainIdxMap:      make(map[int64]soterutil.Address),
	...
	root, err := newRootSoterAddress(s, rootkey, nil, chaincode,
		createdAt)
	if err != nil {
		return nil, err
	}
...
func (s *Store) ReadFrom(r io.Reader) (n int64, err error) {
	...
	s.chainIdxMap = make(map[int64]soterutil.Address)
...
func (s *Store) writeTo(w io.Writer) (n int64, err error) {
	...
	for _, wAddr := range s.addrMap {
		switch soterAddr := wAddr.(type) {
		case *soterAddress:
			e := &addrEntry{
				addr: *soterAddr,
			}
			copy(e.pubKeyHash160[:], soterAddr.AddrHash())
			if soterAddr.Imported() {
				// No order for imported addresses.
				importedAddrs = append(importedAddrs, e)
			} else if soterAddr.chainIndex >= 0 {
				// Chained addresses are sorted.  This is
				// kind of nice but probably isn't necessary.
				chainedAddrs[soterAddr.chainIndex] = e
			}
		...
		case *scriptAddress:
			...
			copy(e.scriptHash160[:], soterAddr.AddrHash())
...
func (s *Store) Lock() (err error) {
	...
	for _, addr := range s.addrMap {
		if baddr, ok := addr.(*soterAddress); ok {
			_ = baddr.lock()
		}
	}
...
func (s *Store) ChangePassphrase(new []byte) error {
	...
	for _, wa := range s.addrMap {
		// Only soterAddresses currently have private keys.
		a, ok := wa.(*soterAddress)
		if !ok {
			continue
		}
...
func (s *Store) NextChainedAddress(bs *BlockStamp) (soterutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (s *Store) nextChainedAddress(bs *BlockStamp) (soterutil.Address, error) {
	addr, err := s.nextChainedSoterAddress(bs)
	if err != nil {
		return nil, err
	}
...
func (s *Store) ChangeAddress(bs *BlockStamp) (soterutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (s *Store) nextChainedSoterAddress(bs *BlockStamp) (*soterAddress, error) {
	...
	soterAddr, ok := addr.(*soterAddress)
	if !ok {
...
func (s *Store) LastChainedAddress() soterutil.Address {
	s.mtx.RLock()
	defer s.mtx.RUnlock()
...
func (s *Store) extendUnlocked(bs *BlockStamp) error {
	...
	lastAddr, ok := waddr.(*soterAddress)
	if !ok {
	...
	newAddr, err := newSoterAddress(s, privkey, nil, bs, true)
	if err != nil {
		return err
	}
...
func (s *Store) extendLocked(bs *BlockStamp) error {
	...
	newaddr, err := newSoterAddressWithoutPrivkey(s, nextPubkey, nil, bs)
	if err != nil {
		return err
	}
	a = newaddr.Address()
...
func (s *Store) createMissingPrivateKeys() error {
	...
	prevAddr, ok := prevWAddr.(*soterAddress)
	if !ok {
		return errors.New("found non-pubkey chained address")
	}
...
func (s *Store) Address(a soterutil.Address) (WalletAddress, error) {
	s.mtx.RLock()
	defer s.mtx.RUnlock()

	// Look up address by address hash.
	soterAddr, ok := s.addrMap[getAddressKey(a)]
	if !ok {
...
// Net returns the soter network parameters for this key store.
...
func (s *Store) SetSyncStatus(a soterutil.Address, ss SyncStatus) error {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (s *Store) ImportPrivateKey(wif *soterutil.WIF, bs *BlockStamp) (soterutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
	...
	pkh := soterutil.Hash160(wif.SerializePubKey())
	...
	soterAddr, err := newSoterAddress(s, privKey, nil, bs, wif.CompressPubKey)
	if err != nil {
		return nil, err
	}
	soterAddr.chainIndex = importedKeyChainIdx

	// Mark as unsynced if import height is below currently-synced
	// height.
	if len(s.recent.hashes) != 0 && bs.Height < s.recent.lastHeight {
		soterAddr.flags.unsynced = true
	}

	// Encrypt imported address with the derived AES key.
	if err = soterAddr.encrypt(s.secret); err != nil {
		return nil, err
	}

	addr := soterAddr.Address()
	...
	s.addrMap[getAddressKey(addr)] = soterAddr
	s.importedAddrs = append(s.importedAddrs, soterAddr)
...
func (s *Store) ImportScript(script []byte, bs *BlockStamp) (soterutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
	...
	if _, ok := s.addrMap[addressKey(soterutil.Hash160(script))]; ok {
		return nil, ErrDuplicate
	}
...
func (s *Store) ExportWatchingWallet() (*Store, error) {
	...
	ws.keyGenerator = *(kgwc.(*soterAddress))
	...
	for apkh, addr := range s.addrMap {
		if !addr.Imported() {
			// Must be a soterAddress if !imported.
			soterAddr := addr.(*soterAddress)

			ws.chainIdxMap[soterAddr.chainIndex] =
				addr.Address()
		}
		apkhCopy := apkh
...
type WalletAddress interface {
	// Address returns a soterutil.Address for the backing address.
...
func (s *Store) ActiveAddresses() map[soterutil.Address]WalletAddress {
	s.mtx.RLock()
	defer s.mtx.RUnlock()

	addrs := make(map[soterutil.Address]WalletAddress)
	for i := int64(rootKeyChainIdx); i <= s.highestUsed; i++ {
		a := s.chainIdxMap[i]
...
//
// A slice is returned with the soterutil.Address of each new address.
// The blockchain must be rescanned for these addresses.
func (s *Store) ExtendActiveAddresses(n int) ([]soterutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
	...
	addrs := make([]soterutil.Address, n)
	for i := 0; i < n; i++ {
...
func (af *addrFlags) ReadFrom(r io.Reader) (int64, error) {
	...
	// soterwallet shall refuse to open any unencrypted addresses.  This
...
// that soterwallet fits into an unused space left by Armory's key store file
// format.
...
type PubKeyAddress interface {
	...
	PrivKey() (*soterec.PrivateKey, error)
	// ExportPrivKey exports the WIF private key.
	ExportPrivKey() (*soterutil.WIF, error)
}

// newSoterAddress initializes and returns a new address.  privkey must
// be 32 bytes.  iv must be 16 bytes, or nil (in which case it is
// randomly generated).
func newSoterAddress(wallet *Store, privkey, iv []byte, bs *BlockStamp, compressed bool) (addr *soterAddress, err error) {
	if len(privkey) != 32 {
		return nil, errors.New("private key is not 32 bytes")
	}

	addr, err = newSoterAddressWithoutPrivkey(wallet,
...
// newSoterAddressWithoutPrivkey initializes and returns a new address with an
...
// randomly generated).
func newSoterAddressWithoutPrivkey(s *Store, pubkey, iv []byte, bs *BlockStamp) (addr *soterAddress, err error) {
	var compressed bool
	switch n := len(pubkey); n {
	case soterec.PubKeyBytesLenCompressed:
		compressed = true
	...
	pk, err := soterec.ParsePubKey(pubkey, soterec.S256())
	if err != nil {
		return nil, err
	}

	address, err := soterutil.NewAddressPubKeyHash(soterutil.Hash160(pubkey), s.netParams())
	if err != nil {
		return nil, err
	}
...
// newRootSoterAddress generates a new address, also setting the
// chaincode and chain index to represent this address as a root
// address.
func newRootSoterAddress(s *Store, privKey, iv, chaincode []byte,
	bs *BlockStamp) (addr *soterAddress, err error) {

	if len(chaincode) != 32 {
		return nil, errors.New("chaincode is not 32 bytes")
	}

	// Create new soterAddress with provided inputs.  This will
	// always use a compressed pubkey.
	addr, err = newSoterAddress(s, privKey, iv, bs, true)
	if err != nil {
		return nil, err
	}
...
// unlocked soterAddress.
func (a *soterAddress) verifyKeypairs() error {
	if len(a.privKeyCT) != 32 {
		return errors.New("private key unavailable")
	}

	privKey := &soterec.PrivateKey{
...
func (a *soterAddress) ReadFrom(r io.Reader) (n int64, err error) {
	var read int64
	...
	addr, err := soterutil.NewAddressPubKeyHash(pubKeyHash[:], a.store.netParams())
	if err != nil {
		return n, err
	}
	a.address = addr
...
func (a *soterAddress) WriteTo(w io.Writer) (n int64, err error) {
	var written int64
...
func (a *soterAddress) encrypt(key []byte) error {
...
func (a *soterAddress) lock() error {
	if !a.flags.encrypted {
...
func (a *soterAddress) unlock(key []byte) (privKeyCT []byte, err error) {
	if !a.flags.encrypted {
	...
	x, y := soterec.S256().ScalarBaseMult(privkey)
...
func (a *soterAddress) changeEncryptionKey(oldkey, newkey []byte) error {
...
func (a *soterAddress) SyncStatus() SyncStatus {
	switch {
...
// PubKeyAddress.
func (a *soterAddress) PubKey() *soterec.PublicKey {
	return a.pubKey
}

func (a *soterAddress) pubKeyBytes() []byte {
	if a.Compressed() {
...
func (a *soterAddress) ExportPubKey() string {
...
func (a *soterAddress) PrivKey() (*soterec.PrivateKey, error) {
...
func (a *soterAddress) ExportPrivKey() (*soterutil.WIF, error) {
	pk, err := a.PrivKey()
	if err != nil {
		return nil, err
	}
	...
	// elsewhere.
	wif, err := soterutil.NewWIF((*soterec.PrivateKey)(pk), a.store.netParams(),
		a.Compressed())
	if err != nil {
		panic(err)
	}
...
// normal key store.
func (a *soterAddress) watchingCopy(s *Store) walletAddress {
	return &soterAddress{
		store:   s,
		address: a.address,
		flags: addrFlags{
...
func (a *soterAddress) setSyncStatus(s SyncStatus) {
	switch e := s.(type) {
	case Unsynced:
...
func newScriptAddress(s *Store, script []byte, bs *BlockStamp) (addr *scriptAddress, err error) {
	...
	scriptHash := soterutil.Hash160(script)

	address, err := soterutil.NewAddressScriptHashFromHash(scriptHash, s.netParams())
	if err != nil {
		return nil, err
	}
...
func (sa *scriptAddress) ReadFrom(r io.Reader) (n int64, err error) {
	...
	address, err := soterutil.NewAddressScriptHashFromHash(scriptHash[:],
		sa.store.netParams())
	if err != nil {
		return n, err
	}
...
// address returns a soterutil.AddressScriptHash for a soterAddress.
func (sa *scriptAddress) Address() soterutil.Address {
	return sa.address
}
...
func (sa *scriptAddress) Addresses() []soterutil.Address {
	return sa.addresses
}
