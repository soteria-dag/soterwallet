...
func TestSoterAddressSerializer(t *testing.T) {
	...
	addr, err := newSoterAddress(fakeWallet, privKey, nil,
		makeBS(0), true)
	if err != nil {
		t.Error(err.Error())
		return
	...
	if !reflect.DeepEqual(addr, &readAddr) {
		t.Error("Original and read soterAddress differ.")
	}
}
...
func TestChaining(t *testing.T) {
	...
	for _, test := range tests {
		...
		// Public keys (used to generate the soter address) MUST match.
		...
		// the next pubkeys.
		pubkeyUncompressed, err := soterec.ParsePubKey(nextPubUncompressedFromPub, soterec.S256())
		if err != nil {
			t.Errorf("%s: Unable to parse next uncompressed pubkey: %v", test.name, err)
			return
		}
		pubkeyCompressed, err := soterec.ParsePubKey(nextPubCompressedFromPub, soterec.S256())
		if err != nil {
			t.Errorf("%s: Unable to parse next compressed pubkey: %v", test.name, err)
			return
		}
		privkeyUncompressed := &soterec.PrivateKey{
			PublicKey: *pubkeyUncompressed.ToECDSA(),
			D:         new(big.Int).SetBytes(nextPrivUncompressed),
		}
		privkeyCompressed := &soterec.PrivateKey{
		...
		if !ok {
			t.Errorf("%s: soterec signature verification failed for next keypair (chained from uncompressed pubkey).",
				test.name)
			return
		}
		...
		if !ok {
			t.Errorf("%s: soterec signature verification failed for next keypair (chained from compressed pubkey).",
				test.name)
			return
		}
	}
...
func TestWalletPubkeyChaining(t *testing.T) {
	...
	if !ok {
		t.Errorf("soterec signature verification failed; address's pubkey mismatches the privkey.")
		return
	}
	...
	ok = sig.Verify(hash, pubKey)
	if !ok {
		t.Errorf("soterec signature verification failed; next address's keypair does not match.")
		return
	}
...
func TestWatchingWalletExport(t *testing.T) {
	...
	var newAddrs []soterutil.Address
	for i := 0; i < 10; i++ {
	...
	pk, _ := soterec.PrivKeyFromBytes(soterec.S256(), make([]byte, 32))
	wif, err := soterutil.NewWIF(pk, tstNetParams, true)
	if err != nil {
		t.Fatal(err)
	}
...
func TestImportPrivateKey(t *testing.T) {
	...
	pk, err := soterec.NewPrivateKey(soterec.S256())
	if err != nil {
	...
	// import priv key
	wif, err := soterutil.NewWIF((*soterec.PrivateKey)(pk), tstNetParams, false)
	if err != nil {
		t.Fatal(err)
	}
