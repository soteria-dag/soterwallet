...
import (
	...
	"github.com/soteria-dag/soterwallet/internal/prompt"
...
func networkDir(dataDir string, chainParams *chaincfg.Params) string {
	...
	// and not "testnet1" or any other version, as the chaincfg testnet1
	// paramaters will likely be switched to being named "testnet1" in the
	// future.  This is done to future proof that change, and an upgrade
	// plan to move the testnet1 data directory can be worked out later.
	if chainParams.Net == wire.TestNet1 {
		netname = "testnet"
	}
...
func convertLegacyKeystore(legacyKeyStore *keystore.Store, w *wallet.Wallet) error {
	...
	for _, walletAddr := range legacyKeyStore.ActiveAddresses() {
		...
		case keystore.PubKeyAddress:
			...
			wif, err := soterutil.NewWIF((*soterec.PrivateKey)(privKey),
