...
var (
	...
	TstRecvTx, _                    = soterutil.NewTxFromBytes(TstRecvSerializedTx)
	...
	TstSpendingTx, _           = soterutil.NewTxFromBytes(TstSpendingSerializedTx)
...
func testStore() (*Store, walletdb.DB, func(), error) {
	...
	err = walletdb.Update(db, func(tx walletdb.ReadWriteTx) error {
		...
		s, err = Open(ns, &chaincfg.TestNet1Params)
		return err
	})
...
func serializeTx(tx *soterutil.Tx) []byte {
...
func TestInsertsCreditsDebitsRollbacks(t *testing.T) {
	...
	dupRecvTx, _ := soterutil.NewTxFromBytes(TstRecvSerializedTx)
	// Switch txout amount to 1 SOTER.  Transaction store doesn't
	...
	TstDoubleSpendTx := soterutil.NewTx(newDupMsgTx)
	...
	TstSpendingTx := soterutil.NewTx(spendingTx)
	...
	tests := []struct {
		...
		},
		{
			...
			bal: 0,
			unc: soterutil.Amount(TstRecvTx.MsgTx().TxOut[0].Value),
			unspents: map[wire.OutPoint]struct{}{
				wire.OutPoint{
					Hash:  *TstRecvTx.Hash(),
					Index: 0,
		...
		},
		{
			...
			bal: soterutil.Amount(TstRecvTx.MsgTx().TxOut[0].Value),
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				wire.OutPoint{
					Hash:  *TstRecvTx.Hash(),
		...
		},
		{
			...
			bal: soterutil.Amount(TstDoubleSpendTx.MsgTx().TxOut[0].Value),
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				wire.OutPoint{
		...
		},
		{
			...
			bal: 0,
			unc: soterutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value),
			unspents: map[wire.OutPoint]struct{}{
				wire.OutPoint{
		...
		},
		{
			...
			bal: 0,
			unc: soterutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value + TstSpendingTx.MsgTx().TxOut[1].Value),
			unspents: map[wire.OutPoint]struct{}{
				wire.OutPoint{
					Hash:  *TstSpendingTx.Hash(),
					Index: 0,
		...
		},
		{
			...
			bal: soterutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value + TstSpendingTx.MsgTx().TxOut[1].Value),
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				wire.OutPoint{
					Hash:  *TstSpendingTx.Hash(),
...
func TestFindingSpentCredits(t *testing.T) {
	...
	expectedBal := soterutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value)
	if bal != expectedBal {
...
func TestCoinbases(t *testing.T) {
	...
	coinbaseMaturity := int32(chaincfg.TestNet1Params.CoinbaseMaturity)

	// Balance should be 0 if the coinbase is immature, 50 SOTER at and beyond
	// maturity.
	//
...
func TestRemoveUnminedTx(t *testing.T) {
	...
	// balance.
	checkBalance := func(expectedBalance soterutil.Amount,
	...
	checkBalance(soterutil.Amount(initialBalance), false)
	checkBalance(soterutil.Amount(initialBalance), true)
	...
	checkBalance(soterutil.Amount(changeAmount), true)
