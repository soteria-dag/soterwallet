...
func (s *Store) addCredit(ns walletdb.ReadWriteBucket, rec *TxRecord, block *BlockMeta, index uint32, change bool) (bool, error) {
	if block == nil {
		...
		v := valueUnminedCredit(soterutil.Amount(rec.MsgTx.TxOut[index].Value), change)
	...
	txOutAmt := soterutil.Amount(rec.MsgTx.TxOut[index].Value)
		...
		for i := range b.transactions {
			...
			if blockchain.IsCoinBaseTx(&rec.MsgTx) {
				...
				for i, output := range rec.MsgTx.TxOut {
					...
					if credKey != nil {
						minedBalance -= soterutil.Amount(output.Value)
...
func (s *Store) Balance(ns walletdb.ReadBucket, minConf int32, syncHeight int32) (soterutil.Amount, error) {
