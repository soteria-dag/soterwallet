...
// Ntxid is the normalized ID of a given soter transaction, which is generated
...
type withdrawalInfo struct {
	...
	lastSeriesID  uint32
	dustThreshold soterutil.Amount
...
func (s *WithdrawalStatus) Fees() soterutil.Amount {
	return s.fees
}
...
// Amount returns the amount (in nanoSoter) in this OutBailmentOutpoint.
func (o OutBailmentOutpoint) Amount() soterutil.Amount {
	return o.amount
}
...
type withdrawalTx struct {
	...
	// struct field instead of a method so that it can be replaced in tests.
	calculateFee func() soterutil.Amount
}
...
func newWithdrawalTx(setOptions func(tx *withdrawalTx)) *withdrawalTx {
	...
	tx.calculateFee = func() soterutil.Amount {
		return soterutil.Amount(1+tx.calculateSize()/1000) * feeIncrement
	}
	setOptions(tx)
	return tx
}
...
func (tx *withdrawalTx) inputTotal() (total soterutil.Amount) {
...
func (tx *withdrawalTx) outputTotal() (total soterutil.Amount) {
...
// addChange adds a change output if there are any nanoSoter left after paying
...
func (p *Pool) StartWithdrawal(ns walletdb.ReadWriteBucket, addrmgrNs walletdb.ReadBucket, roundID uint32, requests []OutputRequest,
	...
	txStore *wtxmgr.Store, txmgrNs walletdb.ReadBucket, chainHeight int32, dustThreshold soterutil.Amount) (
...
func (w *withdrawal) finalizeCurrentTx() error {
	...
	for _, txOut := range tx.outputs {
		...
		amtFulfilled := soterutil.Amount(0)
...
func (w *withdrawal) maybeDropRequests() {
	inputAmount := soterutil.Amount(0)
	for _, input := range w.eligibleInputs {
		inputAmount += input.Amount
	}
	outputAmount := soterutil.Amount(0)
...
func (wi *withdrawalInfo) match(requests []OutputRequest, startAddress WithdrawalAddress,
	lastSeriesID uint32, changeStart ChangeAddress, dustThreshold soterutil.Amount) bool {
...
func getWithdrawalStatus(p *Pool, ns, addrmgrNs walletdb.ReadBucket, roundID uint32, requests []OutputRequest,
	startAddress WithdrawalAddress, lastSeriesID uint32, changeStart ChangeAddress,
	dustThreshold soterutil.Amount) (*WithdrawalStatus, error) {
...
func getRedeemScript(mgr *waddrmgr.Manager, addrmgrNs walletdb.ReadBucket, addr *soterutil.AddressScriptHash) ([]byte, error) {
...
func signMultiSigUTXO(mgr *waddrmgr.Manager, addrmgrNs walletdb.ReadBucket, tx *wire.MsgTx, idx int, pkScript []byte, sigs []RawSig) error {
	...
	redeemScript, err := getRedeemScript(mgr, addrmgrNs, addresses[0].(*soterutil.AddressScriptHash))
	if err != nil {
