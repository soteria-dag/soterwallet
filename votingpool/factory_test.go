...
func TstCreateSeriesCredits(t *testing.T, dbtx walletdb.ReadWriteTx, pool *Pool, seriesID uint32, amounts []int64) []credit {
	...
	for i := range msgTx.TxOut {
		c := wtxmgr.Credit{
			...
			Amount:   soterutil.Amount(msgTx.TxOut[i].Value),
...
func TstNewOutputRequest(t *testing.T, transaction uint32, address string, amount soterutil.Amount,
	net *chaincfg.Params) OutputRequest {
	addr, err := soterutil.DecodeAddress(address, net)
	if err != nil {
...
func TstConstantFee(fee soterutil.Amount) func() soterutil.Amount {
	return func() soterutil.Amount { return fee }
}
...
func createAndFulfillWithdrawalRequests(t *testing.T, dbtx walletdb.ReadWriteTx, pool *Pool, roundID uint32) withdrawalInfo {
	...
	dustThreshold := soterutil.Amount(1e4)
